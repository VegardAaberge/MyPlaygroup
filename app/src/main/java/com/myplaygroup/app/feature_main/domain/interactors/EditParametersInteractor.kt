...
class EditParametersInteractor @Inject constructor(
    ...
    fun MutableList<ParameterItem>.updateError(name: String, validator: (Any) -> ValidationResult){
    ...
    fun <T> List<ParameterItem>.getValue(name: String, value : T) : T {
        return this.first { x -> x.key == name }.value as T
    }

    fun List<ParameterItem>.getItem(name: String) : ParameterItem {
